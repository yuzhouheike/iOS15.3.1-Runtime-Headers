/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/CMCapture.framework/CMCapture
 */

@interface BWDeepFusionProcessorInput : BWStillImageProcessorInput {
    <BWDeepFusionProcessorInputDelegate> * _delegate;
    NSDictionary * _evZeroReferenceFrameAttachments;
    struct { 
        long long value; 
        int timescale; 
        unsigned int flags; 
        long long epoch; 
    }  _evZeroReferenceFramePTS;
    id  _faceObservations;
    struct __CVBuffer { } * _gainMap;
    NSDictionary * _inferenceAttachedMediaMetadata;
    id  _lowResPersonInstanceBoundingBoxes;
    id  _lowResPersonInstanceConfidences;
    id  _lowResPersonInstanceMasks;
    void * _lowResPersonMask;
    struct __CVBuffer { } * _personMask;
    NSArray * _providedInferenceAttachedMedia;
    void * _skinMask;
    id  _skinToneClassifications;
    void * _skyMask;
    struct __CVBuffer { } * _syntheticLong;
    struct __CVBuffer { } * _syntheticLongFusionMap;
    NSDictionary * _syntheticLongFusionMapMetadata;
    NSDictionary * _syntheticLongMetadata;
    struct __CVBuffer { } * _syntheticRealLongFusionMap;
    NSDictionary * _syntheticRealLongFusionMapMetadata;
    struct __CVBuffer { } * _syntheticReference;
    struct __CVBuffer { } * _syntheticReferenceFusionMap;
    NSDictionary * _syntheticReferenceFusionMapMetadata;
    NSDictionary * _syntheticReferenceMetadata;
}

@property (nonatomic, retain) <BWDeepFusionProcessorInputDelegate> *delegate;
@property (nonatomic, copy) NSDictionary *evZeroReferenceFrameAttachments;
@property (nonatomic) struct { long long x1; int x2; unsigned int x3; long long x4; } evZeroReferenceFramePTS;
@property (nonatomic, readonly) NSArray *faceObservations;
@property (nonatomic, readonly) struct __CVBuffer { }*gainMap;
@property (nonatomic, retain) NSDictionary *inferenceAttachedMediaMetadata;
@property (nonatomic, readonly) NSArray *lowResPersonInstanceBoundingBoxes;
@property (nonatomic, readonly) NSArray *lowResPersonInstanceConfidences;
@property (nonatomic, readonly) NSArray *lowResPersonInstanceMasks;
@property (nonatomic, readonly) struct __CVBuffer { }*lowResPersonMask;
@property (nonatomic, readonly) struct __CVBuffer { }*personMask;
@property (nonatomic, retain) NSArray *providedInferenceAttachedMedia;
@property (nonatomic, readonly) struct __CVBuffer { }*skinMask;
@property (nonatomic, readonly) NSArray *skinToneClassifications;
@property (nonatomic, readonly) struct __CVBuffer { }*skyMask;
@property (nonatomic, readonly) struct __CVBuffer { }*syntheticLong;
@property (nonatomic, readonly) struct __CVBuffer { }*syntheticLongFusionMap;
@property (nonatomic, readonly) NSDictionary *syntheticLongFusionMapMetadata;
@property (nonatomic, readonly) NSDictionary *syntheticLongMetadata;
@property (nonatomic, readonly) struct __CVBuffer { }*syntheticRealLongFusionMap;
@property (nonatomic, readonly) NSDictionary *syntheticRealLongFusionMapMetadata;
@property (nonatomic, readonly) struct __CVBuffer { }*syntheticReference;
@property (nonatomic, readonly) struct __CVBuffer { }*syntheticReferenceFusionMap;
@property (nonatomic, readonly) NSDictionary *syntheticReferenceFusionMapMetadata;
@property (nonatomic, readonly) NSDictionary *syntheticReferenceMetadata;

- (void)addBuffer:(struct __CVBuffer { }*)arg1 metadata:(id)arg2 type:(unsigned long long)arg3;
- (void)addPersonMask:(struct __CVBuffer { }*)arg1;
- (void)addSkinMask:(void*)arg1 skyMask:(void*)arg2 lowResPersonMask:(void*)arg3 lowResPersonInstanceMasks:(id)arg4 lowResPersonInstanceConfidences:(id)arg5 lowResPersonInstanceBoundingBoxes:(id)arg6 skinToneClassifications:(id)arg7 faceObservations:(id)arg8;
- (void)dealloc;
- (id)delegate;
- (id)description;
- (id)evZeroReferenceFrameAttachments;
- (struct { long long x1; int x2; unsigned int x3; long long x4; })evZeroReferenceFramePTS;
- (id)faceObservations;
- (struct __CVBuffer { }*)gainMap;
- (id)inferenceAttachedMediaMetadata;
- (id)lowResPersonInstanceBoundingBoxes;
- (id)lowResPersonInstanceConfidences;
- (id)lowResPersonInstanceMasks;
- (struct __CVBuffer { }*)lowResPersonMask;
- (struct __CVBuffer { }*)personMask;
- (id)providedInferenceAttachedMedia;
- (bool)receivedAllInferences;
- (void)setDelegate:(id)arg1;
- (void)setEvZeroReferenceFrameAttachments:(id)arg1;
- (void)setEvZeroReferenceFramePTS:(struct { long long x1; int x2; unsigned int x3; long long x4; })arg1;
- (void)setInferenceAttachedMediaMetadata:(id)arg1;
- (void)setProvidedInferenceAttachedMedia:(id)arg1;
- (struct __CVBuffer { }*)skinMask;
- (id)skinToneClassifications;
- (struct __CVBuffer { }*)skyMask;
- (struct __CVBuffer { }*)syntheticLong;
- (struct __CVBuffer { }*)syntheticLongFusionMap;
- (id)syntheticLongFusionMapMetadata;
- (id)syntheticLongMetadata;
- (struct __CVBuffer { }*)syntheticRealLongFusionMap;
- (id)syntheticRealLongFusionMapMetadata;
- (struct __CVBuffer { }*)syntheticReference;
- (struct __CVBuffer { }*)syntheticReferenceFusionMap;
- (id)syntheticReferenceFusionMapMetadata;
- (id)syntheticReferenceMetadata;

@end
